...
我们将每一步的数值结果总结在表\ref{tab:active-set-eg}中.
...
\begin{table}[H]
    ...
    \hline
    $0$ & $(2, 0)^T$ & $\{ 3, 5 \}$ & $(0, 0)^T$ & $\checkmark$ & $\lambda_3^{(0)} = -2, \lambda_5^{(0)} = -1$ & $3$ \\
    $1$ & $(2, 0)^T$ & $\{ 5 \}$ & $(-1, 0)^T$ & $\checkmark$ & & & $1$ \\
    $2$ & $(1, 0)^T$ & $\{ 5 \}$ & $(0, 0)^T$ & $\checkmark$ & $\lambda_5^{(2)} = -5$ & $5$ \\
    $3$ & $(1, 0)^T$ & $\emptyset$ & $(0, 2.5)^T$ & $\times$ & & & $0.6$ & $1$ \\
    $4$ & $(1, 1.5)^T$ & $\{ 1 \}$ & $(0.4, 0.2)^T$ & $\checkmark$ & & & $1$ \\
    $5$ & $(1.4, 1.7)^T$ & $\{ 1 \}$ & $(0, 0)^T$ & $\checkmark$ & $\lambda_1^{(5)} = 0.8$ \\
    \hline
    \end{tabular}
\end{table}
\end{eg}

我们接下来对积极集法进行进一步的说明, 以阐明一些尚未讨论的问题. 首先是关于初始可行点$\V{x}^{(0)}$的选取
