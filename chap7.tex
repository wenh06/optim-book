...
\begin{figure}
\centering
\begin{tikzpicture}[scale=1.8, circle defined by/.style args={center #1 and point #2}{insert path={let \p1=($(#2)-(#1)$),\n1={veclen(\x1,\y1)} in (#1) circle[radius=\n1]}}]
\coordinate (C) at (1, 2.5);
\draw[-{Stealth}] (-0.5, 0)--(5, 0) node [right] {$x$};
\draw[-{Stealth}] (0, -0.5)--(0, 4.2) node [above] {$y$};
\draw (0, 1) node (p1) [left] {$(0, 1)$} -- (2, 2) node (p2) [above] {$(2, 2)$} -- (4, 1) node (p3) [right] {$(4, 1)$} -- (2, 0) node (p4) [below] {$(2, 0)$};
